...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='16236ce91790d4c75b79f6ce96841db1c843e7d2'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'HEAD, tag: v1.11.9, origin/release-1.11' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'HEAD, tag: v1.11.9, origin/release-1.11' =~ tag:\ (v[^ ,]+) ]]; then
