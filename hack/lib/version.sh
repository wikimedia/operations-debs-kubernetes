...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='a8b52209ee172232b6db7a6e0ce2adc77458829f'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'HEAD, tag: v1.13.12, origin/release-1.13' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'HEAD, tag: v1.13.12, origin/release-1.13' =~ tag:\ (v[^ ,]+) ]]; then
